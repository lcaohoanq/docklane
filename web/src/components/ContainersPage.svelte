...
<section aria-labelledby="containers-title">
    ...
    <div class="title-with-count">
      ...
      <span
        >{search
          ...
          : `${containers.length} discovered`}</span
      >
    </div>
  ...
  {#if loading && containers.length === 0}
    <div class="state-panel card bg-base-100" role="status">
      ...
      <div>
        <h3>Finding containers</h3>
        <p>Inspecting running Docker workloads…</p>
      </div>
    </div>
  {:else if loadError && containers.length === 0}
    <div class="state-panel state-error card bg-base-100" role="alert">
      ...
      <div>
        <h3>Containers are unavailable</h3>
        <p>{loadError}</p>
      </div>
      <button class="btn btn-outline btn-sm" type="button" onclick={onRefresh}>Try again</button>
    </div>
