<script lang="ts">
  ...
  let publishingCount = $derived(
    routes.filter((route) =>
      ["reconciling", "publishing", "verifying"].includes(readinessFor(route).state),
    ).length,
  );
  let attentionCount = $derived(
    routes.filter(
      (route) =>
        ...
        ["unresolved", "ambiguous", "error"].includes(route.observed?.state ?? ""),
    ).length,
  );
  ...
  async function ensureReadiness(route: Route) {
    ...
    try {
      ...
      if (polling.get(route.id) === route.revision) polling.delete(route.id);
    }
  }
  ...
  async function toggle(route: Route) {
    ...
    try {
      ...
      onError(cause instanceof Error ? cause.message : "Route update failed");
    } finally {
      setPending(route.id, false);
    }
  }
  async function remove() {
    ...
    try {
      ...
      onError(cause instanceof Error ? cause.message : "Route deletion failed");
    } finally {
      setPending(route.id, false);
    }
  }
...
{:else}
  <section aria-labelledby="routes-title">
    <header class="page-header">
      ...
      <button class="btn btn-primary primary-action" type="button" onclick={onNewRoute}
        ><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14"></path></svg>New
        route</button
      >
    </header>
    <RouteSummary
      ...
      <div class="title-with-count">
        ...
        <span
          >{search
            ...
            : `syncs every ${reconcileEverySeconds}s`}</span
        >
      </div>
      <div class="panel-toolbar">
        <label class="input search-field"
          ><svg viewBox="0 0 24 24" aria-hidden="true"
            ><circle cx="11" cy="11" r="6"></circle><path d="m16 16 4 4"></path></svg
          ><input
            type="search"
            bind:value={search}
          ...
          />{#if search}<button
              class="btn btn-circle btn-ghost btn-xs clear-search"
              type="button"
            ...
            >{/if}</label
        ><button
          class="btn btn-square btn-outline icon-button secondary"
          type="button"
          ...
          disabled={loading}
          aria-busy={loading}
          >{#if loading}<span class="loading loading-spinner loading-sm"></span>{:else}<svg
              viewBox="0 0 24 24"
              aria-hidden="true"
              ><path d="M20 6v5h-5M4 18v-5h5"></path><path
                d="M6.1 9A7 7 0 0 1 18.7 7.3L20 11M4 13l1.3 3.7A7 7 0 0 0 17.9 15"
              ></path></svg
            >{/if}</button
        >
      </div>
    </div>
    {#if loading && routes.length === 0}<div class="state-panel card bg-base-100" role="status">
        <span class="loading loading-spinner loading-md" aria-hidden="true"></span>
        <div>
          <h3>Loading routes</h3>
          <p>Checking the local controller…</p>
        </div>
      </div>
    {:else if loadError && routes.length === 0}<div
        class="state-panel state-error card bg-base-100"
        role="alert"
      >
        <span class="state-icon">!</span>
        <div>
          <h3>Routes are unavailable</h3>
          <p>{loadError}</p>
        </div>
        <button class="btn btn-outline btn-sm" type="button" onclick={() => onRefresh()}
          >Try again</button
        >
      </div>
    {:else if routes.length === 0}<div class="empty card bg-base-200">
        ...
        <button class="btn btn-primary" type="button" onclick={onNewRoute}>Browse containers</button
        >
      </div>
    ...
    {:else}<RouteList
        ...
        {openMenuId}
        {pendingRouteIds}
        onMenu={(route) => (openMenuId = openMenuId === route.id ? null : route.id)}
        onDiagnose={(route) => {
          openMenuId = null;
...
<ConfirmDialog
  ...
  description={deleteCandidate
    ? `${deleteCandidate.name}.${baseDomain} will stop resolving through Docklane. The container is not changed.`
    : ""}
  confirmLabel="Delete route"
  danger
  alert
  busy={!!deleteCandidate && pendingRouteIds.has(deleteCandidate.id)}
