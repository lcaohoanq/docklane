<script lang="ts">
  ...
  const targetKey = $derived(
    editing
      ? `route:${editing.id}:${editing.revision}`
      : selected
  ...
  const nameConflict = $derived(
    routes.some((route) => route.name === routeName && route.id !== editing?.id),
  );
  ...
  $effect(() => {
    ...
    routeName = editing
      ? editing.name
      : availableRouteName(slug(selected?.composeService || selected?.name || ""), routes);
  ...
  async function save() {
    ...
    try {
      ...
      await onSaved(
        `${routeId ? "Updated" : "Created"} ${routeName}.${baseDomain} · publishing route…`,
        saved.id || routeId || null,
      );
    } catch (cause) {
...
<div class="drawer-layer">
  ...
  >
    <div class="drawer-header">
      ...
      <button
        class="btn btn-square btn-ghost icon-button ghost"
        type="button"
        aria-label="Close route editor"
        onclick={requestClose}
    ...
    <div class="drawer-workload">
      <span>WORKLOAD</span><strong
        >{selected?.composeService ||
          selected?.name ||
          editing?.selector.composeService ||
          "Container"}</strong
      ><small
    ...
    >
      <label
        ...
        {#if issue && (attempted || routeName.length > 0)}<small
            id="route-name-hint"
            class="field-hint error">{issue}</small
          >{:else}<small id="route-name-hint" class="field-hint"
            >Browser URL: https://{routeName || "name"}.{baseDomain}</small
          >{/if}
      </label>
      <div class="field-row">
        <label
          >Internal port<input
            ...
            aria-invalid={routePort < 1 || invalidPort ? "true" : undefined}
            aria-describedby="route-port-hint"
            required
          />
          {#if selected}<datalist id="declared-container-ports"
              >{#each uniquePorts(selected.exposedPorts) as port}<option value={port}
                ></option>{/each}</datalist
            >{/if}
        </label>
        <label
          >Protocol<select class="select" bind:value={routeScheme}
      ...
      {#if selected}<small
          id="route-port-hint"
          class:error={routePort === 0 || invalidPort}
          class="field-hint port-hint">{portRecommendation()}</small
        >{:else}<small id="route-port-hint" class="field-hint port-hint"
          >This is the protocol and port Traefik uses inside Docker. Browser access remains HTTPS.</small
        >{/if}
      <div class="command-preview">
        <div><span>EQUIVALENT CLI</span><code>{command}</code></div>
        <button
          class="btn btn-square btn-ghost icon-button ghost"
          type="button"
          ...
          onclick={copyCommand}
          >{#if commandCopied}<svg viewBox="0 0 24 24" aria-hidden="true"
      ...
      {#if editorError}<div class="alert alert-error editor-error" role="alert">
          <span>{editorError}</span>
        </div>{/if}
      <div class="drawer-actions">
        <button type="button" class="btn btn-outline secondary" onclick={requestClose}
          >Cancel</button
        ><button
          class="btn btn-primary"
          type="submit"
          disabled={saving || nameConflict || routePort < 1 || invalidPort}
          aria-busy={saving}
          >{#if saving}<span class="loading loading-spinner loading-sm"></span>{/if}{saving
            ? "Saving…"
            : editing
              ? "Save changes"
              : "Create route"}</button
...
<ConfirmDialog
  ...
  description="Your route configuration has changed. Closing now will lose those edits."
